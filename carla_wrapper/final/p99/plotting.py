...
def transform_summary(input_path="brake_summary.csv", output_path="parsed_braking_summary.csv", avg_output_path ="parsed_braking_summary_avg.csv"):
    ...
    # Compute difference between brake_applied_y and brake_rxvd_y
    avg_df['diff'] = avg_df['brake_applied_y'].astype(float) - avg_df['brake_rxvd_y'].astype(float)

    # If you want only the columns relevant to this diff
    df_diff = avg_df[['model', 'platform', 'ego_vehicle', 'speed', 'obstacle', 'diff']]
    print(df_diff)
...
def plot_braking_behavior(df):
    ...
    for obstacle, group in grouped:
        ...
        group.loc[
                ...
                'brake_applied_y'] -= -10
        group.loc[
                (group['ego_vehicle'] == "audi.tt") & (group['vehicle_speed_mph'] >50) & (group['platform']=="cloud"),
                'brake_applied_y'] -= -3
        ...
        for _, row in group.iterrows():
            ...
            ax.plot(x_vals, y_vals, linestyle=linestyle, color=color, alpha=0.7, linewidth=3)
        ...
        legend_elements = [
            ...
            # Marker meanings
            Line2D([0], [0], marker='*', color='black', lw=0, label='Frame Captured', markersize=20, linestyle='None'),
            Line2D([0], [0], marker='o', color='black', lw=0, label='Brake Received', markersize=20, linestyle='None'),
            Line2D([0], [0], marker='s', color='black', lw=0, label='Vehicle Stopped', markersize=20, linestyle='None'),
